			...
			<aras-menu
				...
				ref={(node) => {
					...
					node.formatter = (id, item) => {
						const itemTypeName = item.markupHolderItemtypeName;
						const itemType = aras.getItemTypeNodeForClient(
							item.markupHolderItemtypeName
						);
						const itemTypeLabel =
							aras.getItemProperty(itemType, 'label') || itemTypeName;
						const keyedName = `from ${itemTypeLabel} ${item.holderKeyedName}`;
						return (
							<div class="aras-list-item__names">
								<span className="aras-list-item__file-name">{item.label}</span>
								<span className="aras-list-item__keyed-name">{keyedName}</span>
							</div>
						);
					};
